...
def mouse_callback(event, x, y, flags, param):
    global scale, point_loc, point_label, input_stop
    if not input_stop:
        # 获取窗口大小
...
def save_masked_image(image, mask, output_dir, filename, crop_mode_=False):
    if crop_mode_:
        y, x = np.where(mask)
...
while True:
    ...
    selected_mask_resized = None  # 保存resize过后的mask，避免每次都计算
    ...
    if window_width != last_window_width or window_height != last_window_height:
        ...
        scale = min(scale_width, scale_height)
        if selected_mask:
            selected_mask_resized = cv2.resize(selected_mask,
    ...
    if key == 27:
        ...
        if point_loc and point_label:
            ...
            while True:
                ...
                selected_image = cv2.resize(selected_image, (int(original_width * scale), int(original_height * scale)))
        ...
        save_masked_image(image, selected_mask, output_dir, filename)
